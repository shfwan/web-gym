@extends('layouts.main')

@section('content')
    <div class="container mx-auto max-w-7xl  flex items-center justify-center min-h-screen">
        <div class="card bg-white shadow border text-neutral-content w-96">
            <div class="card-body items-center text-center gap-4 space-y-4">
                <h2 class="card-title text-black text-2xl">Failed Checkout</h2>
                <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" fill="red" class="bi bi-x-circle" viewBox="0 0 16 16">
                    <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                    <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
                  </svg>
                  <h1 class="text-black text-base">Kartu member kamu sudah kadaluarsa / Tidak berlaku</h1>
                  <div class="card-actions justify-end">
                    <button class="btn btn-success text-white" onclick="window.location.href='{{ route('upgrade') }}'">Upgrade Sekarang</button>
                </div>
            </div>
        </div>
    </div>
@endsection
