        ...
        <div class="flex flex-col items-center justify-center h-full ">
            <div class="block space-y-4 place-items-center">
                <figure class="max-w-52 cursor-pointer hover:shadow-md rounded-full transition-all" onclick="file.click()">
                    <figure class="bg-gray-400 rounded-full overflow-hidden">
                ...
                <x-input disabled="{{ $status }}" value="{{ $user->firstname }}" label="Nama depan" name="firstname"
                    type="text" placeholder="Nama depan" />
                <x-input disabled="{{ $status }}" value="{{ $user->lastname }}" label="Nama belakang" name="lastname"
                    type="text" placeholder="Nama belakang" />
                <x-input disabled="{{ $status }}" value="{{ $user->email }}" label="Email" name="email"
                    type="text" placeholder="Email" />
                <x-input disabled="{{ $status }}" value="{{ $user->phone }}" label="Nomor HP" name="phone"
