    ...
    <div
        ...
        <div class="block w-full space-y-8 place-items-start">
            ...
            <div class="flex flex-col gap-4 w-full max-w-7xl">
                <div
                    class="w-full grid grid-cols-7 grid-rows-1 border border-gray-300 p-4 gap-4 rounded-md place-items-center">
                    <h3 class="text-black font-semibold place-self-start truncate">
                        {{-- {{ Auth::user()->role == 'member' ? 'Nama Pelatih / Kartu Member' : 'Nama Member' }} --}}
                        {{ Auth::user()->role == 'member' ? 'Detail transaksi' : 'Nama Member' }}
                    </h3>
                    ...
                    <div
                        ...
                        <div class="place-self-start flex items-center justify-center h-full">
                            @if (auth()->user()->role == 'admin')
                                <h3 class="text-black truncate max-w-48">
                                    {{ $item->user->firstname . ' ' . $item->user->lastname }}</h3>
                            @else
                                @if ($item->type == 'Booking')
                                    <div class="">
                                        @if ($item->pelatih)
                                            <h3 class="text-black truncate max-w-48">{{ $item->pelatih->name }}</h3>
                                        @else
                                            <h3 class="text-error font-bold truncate ">Sudah dihapus</h3>
                                        @endif
                                    </div>
                                @elseif ($item->type == 'Card Member')
                                    <h3 class="text-black truncate max-w-44">
                                ...
                                @else
                                    ...
                                    <x-modal id="detailTransaksi{{ $item->id }}" title="Detail Transaksi">
                                        <div class="flex flex-col gap-4  min-w-96 cursor-default">
                                            ...
                                            @if ($item->type == 'Booking')
                                                <x-label type="text" title="{{ $item->type == 'Card Member' ? 'Nama Kartu Member' : 'Nama Pelatih' }}"
                                                    value="{{ $item->pelatih ? $item->pelatih->name : 'Sudah dihapus' }}" />
                                            @elseif ($item->type == 'Card Member')
                                                <x-label type="text" title="{{ $item->type == 'Card Member' ? 'Kartu Member' : 'Nama Pelatih' }}"
                        ...
                        @else
                            <h3 class="text-error font-bold place-self-center truncate text-ellipsis">Product tidak valid
                            </h3>
                        @endif
                    </div>
                @endforeach
